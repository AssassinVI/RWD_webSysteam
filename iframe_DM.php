...
<?php
 $rel_sort=htmlspecialchars($_POST['rel_sort']);
   ...
   if (empty($_GET['funId'])) {
       ...
       /*################# 給聯絡我們 ID ##################*/    
          $result = db_conn("SELECT fun_id FROM Related_tb WHERE fun_id LIKE 'dm".date('Ymd')."%' ORDER BY fun_id DESC LIMIT 1" );
          if (mysql_num_rows($result)<1) {

              $fun_id='dm'.date('Ymd').'001';
           }else{
             ...
             while ($row=mysql_fetch_array($result)) {
              ...
              $sum=$fun_id_down+1;
                    ...
                    $fun_id='dm'.date('Ymd').$sum;
          }
      }
  }
  else{
  	$fun_id=htmlspecialchars($_GET['funId']);
   //  $result=db_conn("SELECT call_name, call_ph, call_em, call_q, call_ti, call_con, call_mu FROM call_us_tb WHERE fun_id='$fun_id' " );
                         ...
                         <div class="row">
                            <form method="POST" action="rwd_php_sys.php" class="form-horizontal">
                              <div class="form-group">
                                ...
                                <button id="cr_btn"  class="btn btn-primary dim  " type="submit" > 產生索取DM按鈕</button>
                                </div>
                                </div>
...
?>
                                ...
                                <input  type="hidden" name="page" value="catch_DM">
